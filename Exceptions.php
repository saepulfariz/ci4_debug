...
class Exceptions
{
    ...
    public function exceptionHandler(Throwable $exception)
    {
        ...
        if (!is_cli()) {
            // echo 'TEST AJAX';
            $this->saveLog($exception, $statusCode);
    ...
    public function saveLog($exception, $statusCode)
    {
        ...
        $res_request = [
            // 'path' => esc($request->getUri()),
            'path' => $protocol . $_SERVER['REQUEST_URI'],
            'http_method' => esc(strtoupper($request->getMethod())),
            'ip_address' => esc($request->getIPAddress()),
            'is_ajax' => ($request->isAJAX()) ? true : false,
            'is_cli' => ($request->isCLI()) ? true : false,
            'is_secure_request' => ($request->isSecure()) ? true : false,
            'user_agent' => esc($request->getUserAgent()->getAgentString()),
        ];
        ...
        if (!empty($headers)) {
            foreach ($headers as $header) {
                $res_headers[esc($header->getName(), 'html')] = esc($header->getValueLine(), 'html');
            }
        }
        ...
        if (!empty($headers)) {
            ...
            foreach (array_keys($headers) as $name) {
                $res_response[esc($name, 'html')] = esc($response->getHeaderLine($name), 'html');
            }
        }
        ...
        foreach ($files as $file) {
            $res_files[] = esc(clean_path($file));
        }

        $res_memory = [
            'memory_usage' => esc(static::describeMemory(memory_get_usage(true))),
            'peak_memory_usage' => esc(static::describeMemory(memory_get_peak_usage(true))),
            'memory_limit' => esc(ini_get('memory_limit')),
        ];
        ...
        foreach (['_SERVER', '_SESSION'] as $var) {
            ...
            foreach ($GLOBALS[$var] as $key => $value) {
                $res_server[$key] = $value;
            }
        }


        $vars['server'] = $res_server;
        $vars['request'] = $res_request;
        $vars['headers'] = $res_headers;
        $vars['response'] = $res_response;
        $vars['files'] = $res_files;
        $vars['memory'] = $res_memory;

        $data_log = json_encode($vars);

        // $this->sendMail($data_log);

        // $home = new \App\Controllers\Home();
        // $home->insert($data_log);

        $myfile = fopen('result_' . date('ymd_his') . '.json', 'w');
        fwrite($myfile, $data_log);
        fclose($myfile);
    }

    public function sendMail($data_log)
    {
        // API URL
        $url = 'http://localhost:8989/debug';
